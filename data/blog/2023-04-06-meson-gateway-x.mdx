---
title: 'Meson Gateway X: Paving the Way for Mass Adoption of Web3 Infrastructure'
date: '2022-12-07'
tags: ['Meson Labs', 'Gateway X', 'Web3']
draft: false
...
Meson, as a bandwidth market service, provides Gateway acceleration services (aka CDN) for Web3 storage facilities. However, in our practice, we often find issues with the origin of the services. Let's take the two major mainstream Web3 storage systems as examples. For IPFS, the gateway is ipfs.io. When users configure ipfs.io as the source on the Meson side, Meson is abstracted as a cache layer, which to some extent alleviates the direct request pressure on the source gateway. However, the fundamental issue lies in the imbalance of supply and demand for a single gateway.
